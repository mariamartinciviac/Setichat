...
public class LoginActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mNick = getIntent().getStringExtra(EXTRA_EMAIL);
		mNickView = (EditText) findViewById(R.id.nickname);
		mNickView.setText(mNick);
	...
	public void attemptLogin() {
		...
		if (TextUtils.isEmpty(mTelephone)) {
			...
			cancel = true;
		} else if (mTelephone.length() < 4) {
			mTelephoneView.setError(getString(R.string.error_invalid_password));
			focusView = mTelephoneView;
			cancel = true;
		}
	...
	public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
		...
		protected Boolean doInBackground(Void... params) {
			...
			for (String credential : DUMMY_CREDENTIALS) {
				...
				if (pieces[0].equals(mNick)) {
					// Account exists, return true if the password matches.
					return pieces[1].equals(mTelephone);
				}
			}

			// TODO: register the new account here.
			SharedPreferences.Editor editor = getSharedPreferences("UserLoggedIn", MODE_PRIVATE).edit();
